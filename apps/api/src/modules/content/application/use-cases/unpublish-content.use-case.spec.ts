...
import { UnpublishContentUseCase } from './unpublish-content.use-case';

const NOW = new Date('2026-06-01T00:00:00.000Z');
const ID = '123e4567-e89b-42d3-a456-426614174000';

function content(status = ContentStatus.Published): Content {
  return Content.reconstitute({
    ...
    publishedAt: NOW, deletedAt: null, createdAt: NOW, updatedAt: NOW,
  });
}
...
function setup(contents = new FakeContents()) {
  ...
  return { events, useCase: new UnpublishContentUseCase(contents, { now: () => NOW }, events, { run: async (fn) => fn() }) };
}

describe('UnpublishContentUseCase', () => {
  it('transitions Published to Unpublished and publishes event', async () => {
    const state = setup();
  ...
  it.each([ContentStatus.Draft, ContentStatus.InReview, ContentStatus.Archived])('throws InvalidTransitionError from %s', async (status) => {
    const contents = new FakeContents();
