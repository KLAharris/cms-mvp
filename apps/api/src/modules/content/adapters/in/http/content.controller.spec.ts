...
class InMemoryContentVersionRepository implements ContentVersionRepository {
  ...
  async findByContentId(contentId: ContentId): Promise<ContentVersion[]> {
    return this.rows.filter((row) => row.contentId.equals(contentId));
  }
  ...
  async nextVersionNo(contentId: ContentId): Promise<number> {
    return this.rows.filter((row) => row.contentId.equals(contentId)).length + 1;
  }
}
...
describe('ContentController', () => {
  ...
  beforeEach(async () => {
    ...
    const module = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: DOMAIN_EVENT_PUBLISHER, useValue: { publishAll: async () => undefined } },
  ...
  it('POST /api/admin/content creates content for author/editor/admin and validates auth/body', async () => {
    ...
    await request(app.getHttpServer()).post('/api/admin/content').send({}).expect(401);
    await request(app.getHttpServer())
      .post('/api/admin/content')
      .set('Authorization', `Bearer ${token}`)
  ...
  it('GET /api/admin/content returns list envelope, filters, and author scoping', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect((response) => {
        ...
        expect(response.body.pagination).toMatchObject({ page: 1, page_size: 25, total: 1 });
      });
  ...
  it('GET /api/admin/content/:id enforces visibility and not found', async () => {
    ...
    await request(app.getHttpServer()).get(`/api/admin/content/${IDS[0]!}`).set('Authorization', `Bearer ${editorToken}`).expect(200);
    await request(app.getHttpServer()).get(`/api/admin/content/${IDS[0]!}`).set('Authorization', `Bearer ${authorToken}`).expect(200);
    await request(app.getHttpServer()).get(`/api/admin/content/${IDS[0]!}`).set('Authorization', `Bearer ${otherAuthorToken}`).expect(403);
    await request(app.getHttpServer()).get(`/api/admin/content/${IDS[5]!}`).set('Authorization', `Bearer ${editorToken}`).expect(404);
    await request(app.getHttpServer()).get(`/api/admin/content/${IDS[0]!}`).expect(401);
  });
  ...
  it('PATCH /api/admin/content/:id updates and maps conflicts', async () => {
    ...
    await request(app.getHttpServer()).patch(`/api/admin/content/${IDS[0]!}`).set('Authorization', `Bearer ${authorToken}`).send({ title: 'Updated', seoTitle: 'New SEO' }).expect(200).expect((response) => {
      expect(response.body.data.title).toBe('Updated');
      expect(response.body.data.seoTitle).toBe('New SEO');
    });
    await request(app.getHttpServer()).patch(`/api/admin/content/${IDS[0]!}`).set('Authorization', `Bearer ${authorToken}`).send({ seoDescription: 'New description' }).expect(200).expect((response) => {
      expect(response.body.data.seoDescription).toBe('New description');
    });
    await request(app.getHttpServer()).patch(`/api/admin/content/${IDS[0]!}`).set('Authorization', `Bearer ${otherAuthorToken}`).send({ title: 'Denied' }).expect(403);
    await request(app.getHttpServer()).patch(`/api/admin/content/${IDS[0]!}`).set('Authorization', `Bearer ${authorToken}`).send({ slug: 'second' }).expect(409);
    await request(app.getHttpServer()).patch(`/api/admin/content/${IDS[0]!}`).set('Authorization', `Bearer ${authorToken}`).send({ slug: 'not valid' }).expect(400);
    await request(app.getHttpServer()).patch(`/api/admin/content/${IDS[5]!}`).set('Authorization', `Bearer ${authorToken}`).send({ title: 'Missing' }).expect(404);
    await request(app.getHttpServer()).patch(`/api/admin/content/${IDS[0]!}`).send({ title: 'No token' }).expect(401);
  });
  ...
  it('state change endpoints submit, publish, unpublish, schedule, and delete', async () => {
    ...
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/submit`).set('Authorization', `Bearer ${authorToken}`).expect(200);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/submit`).set('Authorization', `Bearer ${authorToken}`).expect(409);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[1]!}/publish`).set('Authorization', `Bearer ${editorToken}`).expect(200);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[2]!}/publish`).set('Authorization', `Bearer ${adminToken}`).expect(409);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[2]!}/unpublish`).set('Authorization', `Bearer ${editorToken}`).expect(200);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[1]!}/publish`).set('Authorization', `Bearer ${authorToken}`).expect(403);

    const invalid = seedContent(IDS[3]!, 'author-1', ContentStatus.Draft, 'Invalid');
    invalid.body = null;
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[3]!}/publish`).set('Authorization', `Bearer ${editorToken}`).expect(422);

    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/schedule`).set('Authorization', `Bearer ${editorToken}`).send({ scheduledAt: FUTURE.toISOString() }).expect(200);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/schedule`).set('Authorization', `Bearer ${authorToken}`).send({ scheduledAt: FUTURE.toISOString() }).expect(403);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/schedule`).set('Authorization', `Bearer ${editorToken}`).send({ scheduledAt: NOW.toISOString() }).expect(422);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/schedule`).set('Authorization', `Bearer ${editorToken}`).send({}).expect(400);

    await request(app.getHttpServer()).delete(`/api/admin/content/${IDS[0]!}`).set('Authorization', `Bearer ${editorToken}`).expect(204);
    await request(app.getHttpServer()).delete(`/api/admin/content/${IDS[1]!}`).set('Authorization', `Bearer ${authorToken}`).expect(403);
    await request(app.getHttpServer()).delete(`/api/admin/content/${IDS[5]!}`).set('Authorization', `Bearer ${editorToken}`).expect(404);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/submit`).expect(401);
  });
  ...
  it('versions and revert endpoints enforce permissions and missing resources', async () => {
    ...
    await request(app.getHttpServer()).get(`/api/admin/content/${IDS[0]!}/versions`).set('Authorization', `Bearer ${authorToken}`).expect(200).expect((response) => {
      expect(response.body.data[0].versionNo).toBe(2);
    });
    await request(app.getHttpServer()).get(`/api/admin/content/${IDS[0]!}/versions`).set('Authorization', `Bearer ${otherAuthorToken}`).expect(403);
    await request(app.getHttpServer()).get(`/api/admin/content/${IDS[5]!}/versions`).set('Authorization', `Bearer ${editorToken}`).expect(404);
    await request(app.getHttpServer()).get(`/api/admin/content/${IDS[0]!}/versions`).expect(401);

    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/revert/2`).set('Authorization', `Bearer ${authorToken}`).expect(200).expect((response) => {
      expect(response.body.data.newVersionNo).toBe(3);
    });
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[5]!}/revert/1`).set('Authorization', `Bearer ${editorToken}`).expect(404);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/revert/99`).set('Authorization', `Bearer ${editorToken}`).expect(404);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/revert/1`).set('Authorization', `Bearer ${otherAuthorToken}`).expect(403);
    await request(app.getHttpServer()).post(`/api/admin/content/${IDS[0]!}/revert/1`).expect(401);
  });
});
